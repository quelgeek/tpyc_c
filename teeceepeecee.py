...
class IngresData():
    '''container for Ingres data values'''

    def __init__(self,*args):
        self.descriptor = py.IIAPI_DESCRIPTOR()
        self.datavalue = py.IIAPI_DATAVALUE()
        self.datavalue.dv_null = False
        self.buffer = None

    
    def with_null(self):
        self.descriptor.ds_nullable = True
        return self


    def not_null(self):
        self.descriptor.ds_nullable = False
        return self


    def set_null(self):
        if self.descriptor.ds_nullable:
            self.datavalue.dv_null = True
        else:
            raise NullabilityError
        return self


class Char(IngresData):
    '''Ingres CHAR(n)'''

    def __init__(self,*args):
        super().__init__(self,*args)
        self.descriptor.ds_dataType = py.IIAPI_CHA_TYPE
        nargs = len(args)
        if nargs == 2:
            value = args[0]
            length = len(value)
            ...
            if type(args[0]) is str:
                ...
                length = 0
                extent = args[0]
        self.descriptor.ds_length = extent
        self.datavalue.dv_length = length
        self.buffer = ctypes.c_buffer(extent)
        padding = ' '
        value = value + (padding * extent) 
        value = value[:extent]
        value = value.encode()
        self.buffer.value = value
        self.datavalue.dv_value = ctypes.addressof(self.buffer)
        

class Decimal(IngresData):
    '''VERY rough-n-ready Ingres DECIMAL(p,s)'''
    def __init__(self,*args):
        super().__init__(self,*args)
        self.descriptor.ds_dataType = py.IIAPI_DEC_TYPE
        nargs = len(args)
        if nargs == 3:
            value = args[0]
            ...
            precision = args[0]
            scale = args[1]
        extent = precision // 2 + 1
        sign = 0xD if v < 0.0 else 0xC
        s = str(abs(v))
        whole,fraction = s.split('.')
        whole = ('0' * (precision-scale)) + whole
        whole = whole[-(precision-scale):]
        fraction = fraction + ('0' * scale)
        fraction = fraction[:scale]
        whole = whole.encode()
        fraction = fraction.encode()
        w = [d & 0x0F for d in whole]
        f = [d & 0x0F for d in fraction] + [sign]
        wf = w + f
        if extent % 2 == 1:
            wf = [0] + wf
        content = bytearray(extent)
        for i in range(extent):
            content[i] = wf[i*2] << 4 | wf[i*2+1]
        self.descriptor.ds_length = extent
        self.descriptor.ds_precision = precision
        self.descriptor.ds_scale = scale
        self.datavalue.dv_length = extent
        self.buffer = ctypes.c_buffer(extent)
        self.buffer.value = content
        self.datavalue.dv_value = ctypes.addressof(self.buffer)


class Integer(IngresData):
    '''Ingres INTEGER4'''

    def __init__(self,*args):
        super().__init__(self,*args)
        self.descriptor.ds_dataType = py.IIAPI_INT_TYPE
        nargs = len(args)
        if nargs == 1:
            value = int(args[0])
        elif nargs == 0:
            value = 0
        self.descriptor.ds_length = 4
        self.datavalue.dv_length = 4
        self.buffer = ctypes.c_int(value)
        self.datavalue.dv_value = ctypes.addressof(self.buffer)


class Varchar(IngresData):
    '''Ingres VARCHAR(n)'''

    def __init__(self,*args):
        super().__init__(self,*args)
        self.descriptor.ds_dataType = py.IIAPI_VCH_TYPE
        if nargs == 2:
            value = args[0]
            length = len(value)
            extent = int(args[1]) + 2
        ...
        self.datavalue.dv_length = length
        self.buffer = ctypes.c_buffer(extent)
        self._format = f'h{extent}s'
        value = value.encode()
        struct.pack_into(self._format,self.buffer,0,length,value)
        self.datavalue.dv_value = ctypes.addressof(self.buffer)


    @property
    def value(self):
        _length,_value = struct.unpack(self.format,self.buffer)
        return str(value[:_length])
...
#SQL_by_name = {
#    'stmtGetCustWhse' : b'''
#        SELECT float4(c.discount) as discount,
#               c.last, c.credit, float4(w.tax) as tax 
...
#class Char(ctypes.Structure):               # 20
#    _fields_ = [
#        ('value',ctypes.c_char * 20) ]
#
#class Decimal(ctypes.Structure):            # 10
#    _fields_ = [
#        ('value',ctypes.c_char * 12) ]
#
#class Float(ctypes.Structure):              # 31
#    _fields_ = [
#        ('value',ctypes.c_float) ]
#
#class Integer(ctypes.Structure):            # 30
#    _fields_ = [
#        ('value',ctypes.c_int) ]
#
#class Long_Byte_LOB(ctypes.Structure):      # 35
#    _fields_ = [
#        ('value',ctypes.c_uint) ]
#
#class Timestamp(ctypes.Structure):          # 9
#    _fields_ = [
#        ('value',ctypes.c_char * 14) ]
#
#class Varchar(ctypes.Structure):            # 21
#    _fields_ =  [
#        ('length', ctypes.c_short),
#        ('value', ctypes.c_char * 500) ]
...
#_pattern_lookup = {
#    py.IIAPI_HNDL_TYPE: 'i',
#    py.IIAPI_CHR_TYPE: '{}s',
#    py.IIAPI_CHA_TYPE: '{}s',
#    py.IIAPI_VCH_TYPE: '{}s',
#    py.IIAPI_LVCH_TYPE: 'x',
#    py.IIAPI_LCLOC_TYPE: 'l',
#    py.IIAPI_BYTE_TYPE: '{}s',
#    py.IIAPI_VBYTE_TYPE: '{}s',
#    py.IIAPI_LBYTE_TYPE: 'x',
#    py.IIAPI_LBLOC_TYPE: 'l',
#    py.IIAPI_NCHA_TYPE: '{}h',
#    py.IIAPI_NVCH_TYPE: '{}h',
#    py.IIAPI_LNVCH_TYPE: 'x',
#    py.IIAPI_LNLOC_TYPE: 'l',
#    py.IIAPI_TXT_TYPE: '{}s',
#    py.IIAPI_LTXT_TYPE: '{}s',
#    py.IIAPI_INT_TYPE: '{}s',
#    py.IIAPI_FLT_TYPE: '{}s',
#    py.IIAPI_MNY_TYPE: '8s',
#    py.IIAPI_DEC_TYPE: '{}s',
#    py.IIAPI_BOOL_TYPE: 'cxxx',
#    py.IIAPI_UUID_TYPE: '16s',
#    py.IIAPI_IPV4_TYPE: '4s',
#    py.IIAPI_IPV6_TYPE: '16s',
#    py.IIAPI_DTE_TYPE: '12s',
#    py.IIAPI_DATE_TYPE: '4s',
#    py.IIAPI_TIME_TYPE: '10sxx',
#    py.IIAPI_TMWO_TYPE: '10xxs',
#    py.IIAPI_TMTZ_TYPE: '10xxs',
#    py.IIAPI_TS_TYPE: '14sxx',
#    py.IIAPI_TSWO_TYPE: '14sxx',
#    py.IIAPI_TSTZ_TYPE: '14xxs',
#    py.IIAPI_INTYM_TYPE: '3sx',
#    py.IIAPI_INTDS_TYPE: '12s',
#    py.IIAPI_LOGKEY_TYPE: '16s',
#    py.IIAPI_TABKEY_TYPE: '8s' }
#
#
#def _rounded_up_to_4n(i):
#    if i % 4 != 0:
#        comp = 4 - (i % 4)
#        i = i + comp 
#    return i
#
#
#def _pattern_for_ingtype(ingtype,length):
#    '''return struct format template for Ingres type'''
#
#    global _pattern_lookup
#
#    if ingtype == py.IIAPI_INT_TYPE:
#        if length == 1:
#            pattern = 'bxxx'
#        elif length == 2:
#            pattern = 'hxx'
...
#        elif length == 8:
#            pattern = 'l'
#    elif ingtype == py.IIAPI_FLT_TYPE:
#        if length == 4:
#            pattern = 'f'
#        elif length == 8:
#            pattern = 'd'
#    else:
#        try:
#            p = _pattern_lookup[abs(ingtype)]
#        except keyError:
#            p = '{}s'
#
#        ##  insert the field length into the {} if any; otherwise no-op
#        pattern = p.format(length)
#        ##  work out how much padding is needed to align on word boundaries
#        size = struct.calcsize(pattern)
#        padded_size = _rounded_up_to_4n(size)
#        diff = padded_size - size
#        padding = 'xxxx'[:diff]
#        ##  pad the pattern to a word boundary
#        pattern = pattern + padding
#
#    return pattern
#
#
#def _pythonize(value,type):
#
#    IIAPI_VAR_TYPES = {py.IIAPI_VBYTE_TYPE,py.IIAPI_VCH_TYPE,py.IIAPI_NVCH_TYPE}
#    if type in IIAPI_VAR_TYPES:
#        length, = struct.unpack('h',value[0:2])
#        padding_length = len(value) - length - 2
#        padding = f'{padding_length}x' if padding_length else ''
#        if type == py.IIAPI_VBYTE_TYPE:
#            template = f'xx{length}s' + padding
#            value, = struct.unpack(template,value)
#        elif type == py.IIAPI_VCH_TYPE:
#            template = f'xx{length}s' + padding
#            value, = struct.unpack(template,value)
#            value = value.decode() 
#        elif type == py.IIAPI_NVCH_TYPE:
#            template = f'xx{length * 2}s' + padding
#            value, = struct.unpack(template,value)
#
#    return value


#def prepared_SQL(sql,name,for_update=False):
#    '''reformat a repeated query as a prepared query'''
#
#    prepared = f'PREPARE {name} FROM' + ' ' + sql
#    prepared = prepared.replace('${} = ~V','?')
#    if for_update:
#        prepared = prepared + ' ' + 'FOR UPDATE'
#    return prepared
#
#
#def repeated_SQL(sql):
#    '''number the parameters in a repeated query'''
#
#    n = sql.count('${} = ~V')
#    ns = [i for i in range(n)]
#    repeated = sql.format(*ns)
#    return repeated
